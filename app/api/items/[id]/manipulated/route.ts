...
export async function PATCH(req: NextRequest) {
  try {
    ...
    const nowManipulated = !item.manipulated;
    const latestRap = item.priceHistory[0]?.rap ?? null;

    const updated = await prisma.item.update({
      where: { assetId: BigInt(assetId) },
      data: {
        manipulated: nowManipulated,
        // When marking manipulated: record timestamp + RAP for unmark detection
        // When unmarking: clear those fields
        manipulatedAt:  nowManipulated ? new Date() : null,
        manipulatedRap: nowManipulated ? latestRap  : null,
      },
    });
