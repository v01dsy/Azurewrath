...
export default function Deals() {
  ...
  // Track newly added items (assetId not seen before)
  const [newDealIds, setNewDealIds] = useState<Set<string>>(new Set());
  // Track items whose price dropped since last fetch
    ...
    try {
      ...
      if (!isInitialLoad.current) {
        ...
        incoming.forEach(d => {
          ...
          if (!prev) {
            // Brand new item on the board
            newIds.add(d.assetId);
          } else if (d.bestPrice < prev.bestPrice) {
            // Existing item whose price dropped
  ...
  const filtered = useMemo(() => {
    ...
    return result;
  }, [deals, sortKey, sortDir, dealMin, rapMin, rapMax, priceMin, priceMax]);
  ...
  const hasActiveFilters = dealMin !== 10 || rapMin || rapMax || priceMin || priceMax || sortKey !== "deal" || sortDir !== "desc";
          ...
          {filtered.map(item => {
            ...
            return (
              <Link href={`/item/${item.assetId}`} key={item.assetId}>
                ...
                >
                  <h2
                    className="text-base font-bold mb-3 text-left truncate w-full"
                    title={item.name}
                    style={{ color: '#fff', textShadow: '0 1px 4px #000', fontWeight: 600, letterSpacing: '0.01em' }}
                  >
                    {isNew && (
                      <span className="text-xs mr-1.5 bg-green-500 text-black px-1.5 py-0.5 rounded font-bold">NEW</span>
                    )}
                    {isPriceDrop && !isNew && (
                      <span className="text-xs mr-1.5 bg-orange-500 text-black px-1.5 py-0.5 rounded font-bold">↓ DROP</span>
                    )}
                    {item.name}
                  </h2>
